...
def metapath_subgraph(input_feat, input_adj, node_types, metapath, undirected=True):
    input_adj = torch.tensor(input_adj.todense().tolist())
    input_feat = input_feat.squeeze(0)
    node_num = input_feat.shape[0]
    ...
    # aug_input_feat = delete_row_col(input_feat, drop_node_list, only_row=True)
    # aug_input_adj = delete_row_col(input_adj, drop_node_list)
    # aug_input_feat = aug_input_feat.unsqueeze(0)
    # aug_input_adj = sp.csr_matrix(np.matrix(aug_input_adj))
    return input_feat, aug_input_adj
