...
def to_dgl(x, adj, node_types):
    ...
    p_idx = np.where(node_types == 0)[0]
    a_idx = np.where(node_types == 1)[0]
    f_idx = np.where(node_types == 2)[0]

    p_vs_a = adj[p_idx][:, a_idx]
    p_vs_f = adj[p_idx][:, f_idx]

    hg = dgl.heterograph({
        ('paper', 'pa', 'author'): torch.nonzero(p_vs_a, as_tuple=True),
        ('author', 'ap', 'paper'): torch.nonzero(p_vs_a.transpose(1, 0), as_tuple=True),
        ('paper', 'pf', 'field'): torch.nonzero(p_vs_f, as_tuple=True),
        ('field', 'fp', 'paper'): torch.nonzero(p_vs_f.transpose(1, 0), as_tuple=True)
        },
        num_nodes_dict={
            'paper': len(p_idx),
            'author': len(a_idx),
...
def main(args):
    ...
    features_1 = np.zeros((len(np.where(node_types==1)[0]), features.shape[1])) # author
    features_2 = np.zeros((len(np.where(node_types==2)[0]), features.shape[1])) # subject
    ...
    # model_finetune = nn.DataParallel(model_finetune, device)
    ...
    metapath_list = load_metapaths('../data/ACM/metapaths.txt', node_map)
    ...
    augs = ['dropN_metapath', 'dropE_not_on_metapath']
    ...
    print('Micro F1: {:.2f} +- {:.2f}'.format(best_micro_list.mean().item(), best_micro_list.std().item()))
    print('Macro F1: {:.2f} +- {:.2f}'.format(best_macro_list.mean().item(), best_macro_list.std().item()))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-dataset', default='ACMRaw')
