...
def to_dgl(x, adj, node_types):
    ...
    a_idx = np.where(node_types == 0)[0]
    p_idx = np.where(node_types == 1)[0]
    t_idx = np.where(node_types == 2)[0]
    v_idx = np.where(node_types == 3)[0]

    a_vs_p = adj[a_idx][:, p_idx]
    t_vs_p = adj[t_idx][:, p_idx]
    v_vs_p = adj[v_idx][:, p_idx]

    hg = dgl.heterograph({
        ('author', 'ap', 'paper'): torch.nonzero(a_vs_p, as_tuple=True),
        ('paper', 'pa', 'author'): torch.nonzero(a_vs_p.transpose(1,0), as_tuple=True),
        ('term', 'tp', 'paper'): torch.nonzero(t_vs_p, as_tuple=True),
        ('paper', 'pt', 'term'): torch.nonzero(t_vs_p.transpose(1,0), as_tuple=True),
        ('venue', 'vp', 'paper'): torch.nonzero(v_vs_p, as_tuple=True),
        ('paper', 'pv', 'venue'): torch.nonzero(v_vs_p.transpose(1,0), as_tuple=True)
        },
        num_nodes_dict={
            'author': len(a_idx),
            'paper': len(p_idx),
...
def main(args):
    ...
    features_1 = np.zeros((len(np.where(node_types==1)[0]), features.shape[1])) # paper
    features_2 = np.zeros((len(np.where(node_types==2)[0]), features.shape[1])) # term
    features_3 = np.zeros((len(np.where(node_types==3)[0]), features.shape[1])) # venue
    ...
    if args['hetero']:
        ...
        model = HAN(meta_paths=[['ap', 'pa'], ['ap', 'pt', 'tp', 'pa'], ['ap', 'pv', 'vp', 'pa']],
    ...
    model_finetune = nn.DataParallel(model_finetune, device_ids=[0,1])
    ...
    metapath_list = load_metapaths('../data/DBLP/metapaths.txt', node_map)
    metapath = 'author,paper,author'.split(',')
    metapath = np.array([node_map[n_type.strip()] for n_type in metapath])
    augmentor = Augmentor(aug_ratio=0.4, node_types=node_types, metapath=metapath, metapath_list=metapath_list)
    augs = ['dropN_metapath', 'dropN_not_on_metapath']
    ...
    for i in range(10):
        ...
        for epoch in range(args['num_epochs']):
            ...
            if coin_flip < 0.5:
                ...
                node_features = torch.as_tensor(np.concatenate((features.cpu(), features_1, features_2, features_3), axis=0))
                adj = torch.as_tensor(adj, dtype=int)
        ...
        best_micro_list.append(100*best_micro_f1)
        best_macro_list.append(100*best_macro_f1)


    best_micro_list = np.stack(best_micro_list)
    best_macro_list = np.stack(best_macro_list)

    print('Micro F1: {:.4f} +- {:.4f}'.format(best_micro_list.mean().item(), best_micro_list.std().item()))
    print('Macro F1: {:.4f} +- {:.4f}'.format(best_macro_list.mean().item(), best_macro_list.std().item()))
