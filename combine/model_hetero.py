...
class ContrastLayer(nn.Module):
    ...
    def forward(self, feat, pos, neg):
        ...
        #
        # l_pos = f(l_pos)
        # l_neg = f(l_neg).sum(1).unsqueeze(1)
        #
...
class HANLayer(nn.Module):
    ...
    def forward(self, g, h, opt, layer_number, epoch, get_attention=True, no_loss=False):
        ...
        if layer_number == 0:
            project_embeddings = self.semantic_attention(torch.stack(project_embeddings, dim=1))
        else:
            project_embeddings = None

        return self.semantic_attention(semantic_embeddings), project_embeddings                       # (N, D * K)
