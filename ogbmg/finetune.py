...
gnn.load_state_dict(load_dict['state_dict'])
...
for epoch in np.arange(args.n_epoch) + 1:
    ...
    print('Epoch: %d LR: %.5f Train Loss: %.4f Train Acc: %.4f Valid Acc: %.4f' % \
         (epoch,  optimizer.param_groups[0]['lr'], avgs[0], avgs[1], avgs[2]))
    st = time.time()
