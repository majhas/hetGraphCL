...
def main(args):
    ...
    for aug1 in aug_choices:
        ...
        if args.aug is not None:
            if aug1 != args.aug:
                auc_results_table = [0]*len(aug_choices)
                ap_results_table = [0]*len(aug_choices)
        ...
        for aug2 in aug_choices:
            ...
            auc_res.append('{:.2f} +- {:.2f}'.format(aucs.mean().item(), aucs.std().item()))
            ap_res.append('{:.2f} +- {:.2f}'.format(aps.mean().item(), aps.std().item()))

        auc_results_table.append(auc_res)
        ap_results_table.append(ap_res)

    auc_results_table = pd.DataFrame(auc_results_table, columns=aug_choices, index=aug_choices)
    ap_results_table = pd.DataFrame(ap_results_table, columns=aug_choices, index=aug_choices)
    # auc_results_table.to_csv(args.load.replace('.pkl', '_auc_results_table.csv'))
    # ap_results_table.to_csv(args.load.replace('.pkl', '_ap_results_table.csv'))

    print('AUC')
    print(auc_results_table)

    print('\nAP')
    print(ap_results_table)

    # table = create_table(auc_results_table, index=True)
    # table.update_layout(
    #     autosize=False,
    #     width=2400,
    #     height=500
    # )
    # table.write_image(args.load.replace('.pkl', '_auc_semisup_results_table.png'))
    #
    # table = create_table(ap_results_table, index=True)
    # table.update_layout(
    #     autosize=False,
    #     width=2400,
    #     height=500
    # )
    # table.write_image(args.load.replace('.pkl', '_ap_semisup_results_table.png'))
