...
def main(args):
    ...
    train_finetune(model, dataset, criterion, opt, masks, args.epochs, patience=patience, device=device)

    gnn.to('cpu')
    embs = gnn(node_features, adj)
    target_idx = np.where(node_types==0)[0]
    evaluate_clusters(embs[target_idx].detach().numpy(), labels.detach().numpy()[target_idx], n_classes)
